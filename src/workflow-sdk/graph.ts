...
export const newDAG = (flow: Instance): DAG => {
  ...
  for (let action of flow.actions) {
    if (g.hasNode(action.id)) {
      throw new Error(
        `Workflow has two actions with the same ID: ${action.id}`,
      );
    }
  ...
  g.forEachNode((id, attrs) => {
    if (id !== sourceNodeID && g.inEdges(id).length === 0) {
      throw new Error(
        `An action is disconnected and will never run: ${
          attrs?.action?.id || id
        }`,
      );
    }
...
export const bfs = async (
  graph: DAG,
  cb: (node: InstanceAction, edge: InstanceEdge) => Promise<any>,
): Promise<any> => {
  if (graph.order <= 1) {
  ...
  while (queue.length > 0) {
    ...
    for (let node of nodes) {
      ...
      if (!edge || edge?.done == true) {
        ...
        throw new Error(
          `Error finding edge during DAG iteration: ${next} -> ${node.id}`,
        );
      }
